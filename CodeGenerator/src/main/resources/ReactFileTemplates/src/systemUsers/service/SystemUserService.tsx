...
export const useSystemUsersService = () => {
    ...
    const getData: (filterSpecification: FilterData) => Promise<SystemUsersResponseData> = (
        filterSpecification
    ) => {
        return getFilteredObjects<SystemUsersResponseData>(
            API_ROUTES.FILTER_SYSTEM_USERS,
            filterSpecification
        );
    };
    ...
    const deleteObject: (idObject?: number | string) => Promise<boolean> = async (
