...
export const use#{FUL_TABLE_NAME}#Service = () => {
  ...
  const getAll: () => Promise<Response> = () => {
    return getAllObjects(API_ROUTES.ROOT_#{AUL_TABLE_NAME}#);
  };
  ...
  const create: (object: #{FUL_TABLE_NAME}#) => Promise<#{FUL_TABLE_NAME}#> = async (object: #{FUL_TABLE_NAME}#) => {
    return await createObject<#{FUL_TABLE_NAME}#>(API_ROUTES.ROOT_#{AUL_TABLE_NAME}#, object);
  };
  const update: (object: #{FUL_TABLE_NAME}#) => Promise<#{FUL_TABLE_NAME}#> = async (object: #{FUL_TABLE_NAME}#) => {
    ...
    return await updateObject<#{FUL_TABLE_NAME}#>(url, object);
  };
