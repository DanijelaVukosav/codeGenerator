...
const Dropdown: FC<{
  ...
  return (
    <div className="dropdown-container" ref={dropdownRef}>
      ...
      <div className={`options-container ${isPositionBottom ? "bottom" : "top"} ${isOpen ? "show" : ""}`}>
        {options.map((option) => (
          <div key={option} className={`option ${selectedOption === option ? "selected" : ""}`} onClick={() => handleSelect(option)}>
            {option}
          </div>
        ))}
      </div>
