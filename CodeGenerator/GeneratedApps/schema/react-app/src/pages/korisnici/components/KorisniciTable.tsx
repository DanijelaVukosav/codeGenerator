...
import TableRow from "./KorisniciTableRow";
import { Korisnici, KorisniciColumns } from "../types";

export const KorisniciTable: React.FC = () => {
  const {
    isReady,
    hasError,
    sortingColumns,
  ...
  const tableColumns = useMemo(() => {
    return KorisniciColumns.filter((column: TableColumnType) => column.isVisibleOnTable);
  }, []);
  ...
  return !isReady ? (
    ...
    <React.Fragment>
      <IndexTableWrapper>
        ...
        <IndexTableBody>
          {filteredObjects?.map((object: Korisnici, i: number) => {
            return <TableRow key={"korisnici" + i} index={i + 1} data={object} />;
          })}
          {hasError && (
            <tr>
